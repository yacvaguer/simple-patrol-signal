...
{
    [Info("SimplePatrolSignal", "Yac Vaguer", "1.1.0")]
    ...
    {
        [PluginReference("NoEscape")] private Plugin NoEscape;
        ...
        private void LoadDefaultMessages()
        {
            lang.RegisterMessages(new Dictionary<string, string> {
                ...
                ["CooldownActive"] = "You must wait {0} minutes before using another Patrol Heli Signal.",
                ["VIPCooldownActive"] = "[VIP] You must wait {0} minutes before using another Patrol Heli Signal.",
                ...
                ["NoEscapeBlocked"] = "You cannot call a patrol helicopter during combat block."
        ...
        private void LoadData()
        {
            try {
                ...
                storedData.Cooldowns = storedData.Cooldowns
                    .Where(kvp => (currentTime - kvp.Value) < config.Signal.CooldownSeconds)
        ...
        private void CmdHeliSignalChat(BasePlayer player, string command, string[] args)
        {
            ...
            switch (args[0].ToLower()) {
                ...
                if (args.Length > 1) {
                    ...
                    player.ChatMessage(string.Format(GetMessage("CooldownResetTarget", player.UserIDString), target.displayName));
                } else {
        ...
        {
            double currentTime = CurrentTime();
            var expiredCooldowns = storedData.Cooldowns
                .Where(kvp => (currentTime - kvp.Value) >= config.Signal.CooldownSeconds)
                .Select(kvp => kvp.Key)
                .ToList();
        ...
        {
            return HasVIPPermission(player) ? config.Signal.VIPCooldownSeconds : config.Signal.CooldownSeconds;
        }
        ...
        private double CurrentTime() => System.DateTime.UtcNow.Subtract(System.DateTime.UnixEpoch).TotalSeconds;
        ...
        private bool IsRaidBlocked(BasePlayer player)
        {
            if (NoEscape == null || !NoEscape.IsLoaded) {
                PrintWarning("It seems that the NoEscape Plugin is not loaded and you are trying to use it");
                return false;
            }
        ...
        private void OnExplosiveThrown(BasePlayer player, BaseEntity entity)
        {
            if (entity is SupplySignal signal && signal.skinID == config.Signal.SkinId) {
                ...
                if (IsOnCooldown(player)) {
                    ...
                    string messageKey = HasVIPPermission(player) ? "VIPCooldownActive" : "CooldownActive";
                    player.ChatMessage(string.Format(GetMessage(messageKey, player.UserIDString), remainingSeconds));
                    ReturnSignal(player, signal);
                    return;
                }
        ...
        {
            [JsonProperty("Version")] public VersionConfig version { get; set; } = new VersionConfig();


            public class VersionConfig
            {
                [JsonProperty("Major")] public int major { get; set; } = 1;
                [JsonProperty("Minor")] public int minor { get; set; } = 0;
                [JsonProperty("Patch")] public int patch { get; set; } = 0;
            }
